...
class AnimImpl {
public:
    AnimImpl(const std::string& name);
    virtual ~AnimImpl();
...
template <typename T> T AnimImpl::lookup_data(const std::string& name) const {
    const size_t i = std::min(size_t(glm::floor(time)), data_path.at(name).size() - 1);
    return std::any_cast<T>(data_path.at(name)[i]);
}

template <typename T> T AnimImpl::eval_data(const std::string& name) const {
...
using Animation = NamedHandle<AnimImpl>;
