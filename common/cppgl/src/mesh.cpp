...
void MeshImpl::draw_instanced(GLsizei primcount) const {
    if (ibo)
        glDrawElementsInstanced(primitive_type, num_indices, GL_UNSIGNED_INT, 0, primcount);
    else
        glDrawArraysInstanced(primitive_type, 0, num_vertices, primcount);
}
...
void MeshImpl::setup_instancing(const std::vector<glm::mat4> &instance_matrices) {
    if (instance_matrices.empty()) return;

    if (!instance_vbo) instance_vbo = VBO(name + "_instance_buffer");
    instance_vbo->upload_data(instance_matrices.data(), sizeof(glm::mat4) * instance_matrices.size(), GL_DYNAMIC_DRAW);

    glBindVertexArray(vao);
    instance_vbo->bind();

    GLuint base_attrib = vbos.size();

    for (int i = 0; i < 4; i++) {
        GLuint attrib_index = base_attrib + i;
        glEnableVertexAttribArray(attrib_index);
        glVertexAttribPointer(attrib_index, 4, GL_FLOAT, GL_FALSE, sizeof(glm::mat4), (void*)(sizeof(glm::vec4) * i));
...
void MeshImpl::cleanup_instancing() {
    if (!instance_vbo) return;

    glBindVertexArray(vao);

    GLuint base_attrib = vbos.size();
    for (int i = 0; i < 4; i++) {
        glDisableVertexAttribArray(base_attrib + i);
        glVertexAttribDivisor(base_attrib + i, 0);
    }
