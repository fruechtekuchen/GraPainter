...
// Can be paused with GameTimer::pause
struct GameTimer : Timer {

    inline double look() {
        if (paused) {
            pause_offset += pause_timer.look();
            pause_timer.begin();
        }
        return Timer::look() - pause_offset;
    }

    static bool paused;
    static Timer pause_timer;
    static double pause_offset;
    ...
    static inline void unpause() {
        paused = false;
        // Might overflow after very, very long pause maybe.
        pause_offset += pause_timer.look();
    }
};
